...
program
  ...
  .action(async (paths: string[], options: { severity: Severity; json: boolean; failOn?: Severity }) => {
    ...
    if (options.json) {
      ...
      // Summary
      const summary = analyzer.summarize([{ file: '', language: null, patterns: allPatterns, analyzedAt: new Date(), durationMs: 0 }]);
      console.log('\n' + '='.repeat(50));
